...
class WebNewsScanner:
    def __init__(self):
        # Italian insurance and financial news sites
        self.sources = [
            {
                'url': 'https://www.ilgiornale.it/sezioni/economia',
                'title_selector': 'h2.title',
                'content_selector': 'div.excerpt',
            ...
            },
            {
                ...
                'content_selector': 'p.summary-art',
                'date_selector': 'time.time-art',
                'link_selector': 'h3.title-art a'
            }
        ]
                ...
                for title_elem, content_elem, date_elem, link_elem in zip(titles[:20], contents[:20], dates[:20], links[:20]):  # Limit to first 20 articles
                    try:
                        ...
                        for company, variations in list(self.companies.items())[:2]:
                            if any(var.lower() in full_text for var in variations):
                        ...
                        if any(var.lower() in full_text for var in self.companies['alleanza']):
        ...
        return f"""
            <div class="news-item">
                <h3>
                    ...
                    <span class="sentiment {news['sentiment']['label']}">{news['sentiment']['label']}</span>
                </h3>
...
if __name__ == "__main__":
    scanner = WebNewsScanner()
    print("Starting web news scan...")
    scanner.scan_news()
